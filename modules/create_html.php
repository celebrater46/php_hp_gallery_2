...
function get_lang_links($state): string
{
    ...
    $html .= cm\space_br("<a href='" . PHG_INDEX . "?lang=" . ($state->lang === 1 ? "0" : "1") . ($state->category === null ? "" : "&category=" . $state->category) . ($state->pic === null ? "" : "&pic=" . $state->pic) . "'>", 2);
...
//    if(file_exists($img)){
    ...
    $html .= cm\space_br("<a href='" . PHG_INDEX . "?lang=" . $state->lang . "'>" . ($state->lang === 1 ? "BACK" : "戻る") . "</a>", 2);
    $html .= cm\space_br("</div>", 1);
    return $html;
}
